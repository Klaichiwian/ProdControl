...
def process_bom():
    ...
    def find_sub_parts(part_number, qty, due_time):
        stack = [(part_number, qty, due_time)]
        visited = set()
        
        while stack:
            current_part, current_qty, current_due_time = stack.pop()
            if current_part in visited:
                continue
            visited.add(current_part)
            
            sub_parts = bom_df[bom_df['main_partnumber'] == current_part]
            for _, row in sub_parts.iterrows():
                sub_part = row['sub_partnumber']
                sub_qty = row['Sub Qty'] * current_qty
                lead_time = row['Lead Time (sec)']
                start_time = current_due_time - timedelta(seconds=lead_time)
                ...
                stack.append((sub_part, sub_qty, start_time))
